...
from apps.EmotionalMusicGenerator.config import mapping
...
class CreateDatasetFromDataFrame(Dataset):
    def __init__(self, dataset_dir, transform=None, validate=False):
        ...
        self.classes_valid = []
        self.images_valid = []
        for cls in os.listdir(self.dir):
            for im in os.listdir(os.path.join(self.dir, cls)):
                if random() > 0.1:
                    self.classes.append(cls)
                    self.images.append(im)
                else:
                    self.classes_valid.append(cls)
                    self.images_valid.append(im)
        self.transform = transform

    def __len__(self):
        if self.validate:
            return len(self.images_valid)
    ...
    def __getitem__(self, ind):
        ...
        if self.validate:
            label = self.classes_valid[ind]
            image = self.images_valid[ind]
        img = Image.open(os.path.join(self.dir, label, image))
        label = torch.tensor(mapping[label], dtype=torch.long)
